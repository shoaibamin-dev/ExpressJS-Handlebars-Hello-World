...
#### Fixes
1. Break the contents of the HTML file into pieces that follow a logical separation of concerns for the browser. x
2. Fix any invalid HTML x
3. Fix any JS errors / inefficiencies. x
5. Utilize closures to prevent pollution of the global object with app code 

#### Features
1. Make the form keyboard-accessible x
2. Add support for localStorage such that refreshing the page does not reset your task list x
3. Add form validation such that an empty task cannot be submitted.x
4. Convert float-based layouts to flexbox-based layouts. The visuals should not change, just the CSS that handles the layout.x
5. Make the design responsive, such that -
    * The form fills 100% width of the screen up until 375px wide.x
    * The form becomes centered in the page after 375px.x
    * There should be no horizontal scroll bars present at any width.x
